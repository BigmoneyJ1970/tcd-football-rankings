...
export default async function handler(req, res) {
  try {
    ...
    const out = { ok: true };
    if (ap) out.apUrl = (await put('tcd-ap.json', JSON.stringify(ap), { access: 'public', contentType: 'application/json' })).url;
    if (coaches) out.coachesUrl = (await put('tcd-coaches.json', JSON.stringify(coaches), { access: 'public', contentType: 'application/json' })).url;
